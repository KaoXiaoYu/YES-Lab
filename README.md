...
证书上传优先按文件签名识别，并扫描非标准 JPG 的前导字节；对于扫描仪或移动端导出的 `.jpg/.jpeg`，还会结合 `image/jpeg`、`image/jpg`、`image/pjpeg` 或 `application/octet-stream` 做证书专用兼容。非 JPG 扩展名不能借此绕过校验。比赛审核通过后，图片证书会直接作为公开成果详情页的主图展示，PDF 证书直接嵌入页面；审核前不可公开访问。
