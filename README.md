...
招新管理支持多人面试官的线下面试场次、候选人预约/取消、固定顺序面试号、单人叫号、未到场移至队尾、面试结论及提前结束释放队列。讨论板向所有访客公开浏览，正式成员可使用富文本编辑器发布带格式文字、链接、网络图片和代码块的讨论及一级回复，并进行点赞；帖子与回复共用不可回收的连续内容编号，可按发布时间、编号、点赞数和回复数排序。作者头像和姓名可进入其公开成员主页，主页同步展示该成员发布的讨论和回复。互动、面试变更与结果均由站内机器人“梅琳娜”推送，多个新消息在前端折叠提示。成员没有手工发送站内消息的接口。
...
- 当前讨论板升级需同时更新 API 与 Web；API 启动时由 Flyway 自动执行 `V7_1__discussion_content_numbers.sql`，新增编号映射并为既有帖子、回复回填连续编号。建议继续使用现有先备份后更新的部署脚本，无需手工执行迁移 SQL。
