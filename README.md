...
证书上传按文件内容签名识别格式，兼容浏览器把 JPG 标记为 `image/jpg`、`image/pjpeg` 或 `application/octet-stream` 的情况；伪装扩展名的文件仍会被拒绝。比赛审核通过后，公开详情页会提供证书查看入口，审核前不可公开访问。

成员可在“编辑个人主页”的“主页展示内容”中勾选、隐藏并调整本人公开项目和已认证奖项的顺序。项目、比赛、招新面试官和首页成员配置等成员选择入口支持按姓名或学号/内部编号搜索；学号只在登录后的内部接口中使用，不进入公开成员或公开项目数据。
