...
讨论内容连续编号随 `V7_1__discussion_content_numbers.sql` 发布，Flyway 中版本显示为 `7.1`。该迁移只新增编号映射表，并按创建时间为已有帖子和回复回填全局递增编号；不修改 V7 已建表，也不回收删除内容留下的编号。部署时继续先备份再启动 API 自动迁移，不要改名或修改已经执行过的 V7.1 迁移及其校验和。
