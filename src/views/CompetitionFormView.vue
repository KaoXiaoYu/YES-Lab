...
const certificateError = ref(''); const imageError = ref(''); const imageMessage = ref(''); const deletingImageId = ref(null); const existingImageUrls = ref({})
...
async function chooseImages(event) {
  ...
  imageError.value = ''; imageMessage.value = ''
...
async function submit() {
  ...
  try {
    ...
    const savedAssets = [certificate.value ? '证书' : '', imageFiles.value.length ? `${imageFiles.value.length} 张比赛图片` : ''].filter(Boolean)
  ...
  <PortalShell eyebrow="COMPETITION / CAPTAIN SUBMISSION" :title="editing ? '编辑比赛记录' : '队长提交比赛'" description="提交人将自动成为队长。队员既可关联实验室账号，也可只保留公开展示姓名。">
    ...
    <form v-else class="competition-form" @submit.prevent="submit">
      ...
      <section class="competition-form-section"><header><span>03</span><div><p>EVIDENCE & GALLERY</p><h2>证书与比赛图片</h2></div></header><div class="competition-form-grid">
        ...
        <section v-if="editing && existing?.images.length" class="existing-image-gallery full" aria-labelledby="existing-gallery-title"><header><div><strong id="existing-gallery-title">现有比赛图集</strong><small>可单独删除；新选择一组图片并保存时，会替换剩余图集。</small></div><span>{{ existing.images.length }} / 8</span></header><p v-if="imageMessage" class="save-message" role="status">{{ imageMessage }}</p><div><article v-for="image in existing.images" :key="image.id"><div class="existing-image-preview"><img v-if="existingImageUrls[image.id]" :src="existingImageUrls[image.id]" :alt="image.description" /><ImagePlus v-else :size="24" aria-hidden="true" /></div><p>{{ image.description }}</p><button type="button" :disabled="deletingImageId === image.id" :aria-label="`删除图片：${image.description}`" @click="removeExistingImage(image)"><Trash2 :size="16" aria-hidden="true" />{{ deletingImageId === image.id ? '删除中…' : '删除' }}</button></article></div></section>
