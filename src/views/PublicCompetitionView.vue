<script setup>
import { ArrowLeft, CalendarDays, ExternalLink, FileBadge, FolderKanban, Medal, ShieldCheck, UsersRound } from 'lucide-vue-next'
...
<template>
  <div class="public-competition-page">
    ...
    <main v-if="item" class="public-result-main">
      ...
      <div class="public-result-layout"><article class="public-result-story"><p>ABOUT THE RESULT</p><h2>比赛记录</h2><div>{{ item.description }}</div><a v-if="item.certificateUrl" class="public-certificate-link" :href="item.certificateUrl" target="_blank" rel="noopener noreferrer"><FileBadge :size="18" aria-hidden="true" /><span>查看获奖证书<strong>{{ item.certificateOriginalName }}</strong></span><ExternalLink :size="16" aria-hidden="true" /></a></article><aside class="public-result-facts"><div><CalendarDays :size="18" aria-hidden="true" /><span>比赛时间<strong>{{ item.competitionDate }}</strong></span></div><div><UsersRound :size="18" aria-hidden="true" /><span>队长<strong>{{ item.captain.name }}</strong></span></div><div><ShieldCheck :size="18" aria-hidden="true" /><span>指导老师<strong>{{ item.advisorName || '未关联' }}</strong></span></div><div v-if="item.project"><FolderKanban :size="18" aria-hidden="true" /><span>关联项目<strong>{{ item.project.name }}</strong></span></div></aside></div>
