...
<template>
  <div class="public-competition-page">
    ...
    <main v-if="item" class="public-result-main">
      ...
      <section v-if="item.certificateUrl" class="public-result-certificate" aria-labelledby="certificate-title"><header><p>VERIFIED CERTIFICATE</p><h2 id="certificate-title">获奖证书</h2><span>{{ item.certificateOriginalName }}</span></header><figure><img v-if="item.certificateContentType?.startsWith('image/')" :src="item.certificateUrl" :alt="`${item.name}获奖证书`" loading="eager" /><object v-else :data="item.certificateUrl" type="application/pdf" :aria-label="`${item.name}获奖证书 PDF`"><p>当前浏览器无法直接预览证书 PDF，可<a :href="item.certificateUrl" target="_blank" rel="noopener noreferrer">在新窗口打开</a>。</p></object></figure></section>
      <section v-if="item.images.length" class="public-result-gallery"><figure v-for="image in item.images" :key="image.id"><img :src="image.url" :alt="image.description" loading="lazy" /><figcaption>{{ image.description }}</figcaption></figure></section>
